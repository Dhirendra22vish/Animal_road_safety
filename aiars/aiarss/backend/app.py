...
# Set page config
st.set_page_config(
    page_title="UP Animal Road Safety",
    page_icon="🐘",
    layout="wide",
...
def send_mobile_alert(alert_data, alert_type="warning"):
    """Send simulated mobile alert (SMS/Push notification)"""
    
    alert_templates = {
        "warning": {
            "title": "🚨 WILDLIFE WARNING",
            "message": f"Animal crossing zone ahead: {alert_data['zone_name']}. Reduce speed to {alert_data['recommended_speed']} km/h. Distance: {alert_data['distance']}km.",
            "priority": "medium"
        },
        "critical": {
            "title": "🚨 CRITICAL ALERT",
            "message": f"IMMEDIATE DANGER: {alert_data['species'].title()} detected at {alert_data['distance']}km! STOP VEHICLE. Location: {alert_data['zone_name']}.",
            "priority": "high"
        },
        "animal_detected": {
            "title": "🦌 ANIMAL DETECTED",
            "message": f"LIVE ANIMAL: {alert_data['species'].title()} spotted at {alert_data['distance_from_vehicle']}km. Confidence: {alert_data['confidence']*100}%. Take immediate precautions.",
            "priority": "high"
        },
        "emergency": {
            "title": "🚑 EMERGENCY ALERT",
            "message": f"WILDLIFE COLLISION RISK: {alert_data['species'].title()} in immediate path! EMERGENCY BRAKING REQUIRED.",
            "priority": "critical"
        }
    }
...
Animal: {alert_data['species'].title()}
...
def simulate_push_notification(alert_data):
    ...
    notification = {
        ...
        "body": f"{alert_data['species'].title()} detected nearby",
        "data": alert_data
    }
    return notification
...
def main():
    ...
    st.markdown('<h1 class="main-header">🐘 UP Animal Road Safety System</h1>', unsafe_allow_html=True)
    st.markdown('<p class="subtitle">Uttar Pradesh Wildlife Protection • Real-time Collision Prevention • Smart Route Planning</p>', unsafe_allow_html=True)
    ...
    with col_alerts:
        ...
        for species, count in species_counts.items():
            ...
            <div class="species-card">
                ...
                <span>{get_species_emoji(species)} <strong>{species.title().replace('_', ' ')}</strong></span>
            </div>
            """, unsafe_allow_html=True)
            ...
            if st.session_state.mobile_alerts:
                for alert in reversed(st.session_state.mobile_alerts):
                    ...
                    <div style="border: 1px solid #ddd; border-left: 4px solid {priority_color}; padding: 10px; margin: 5px 0; border-radius: 5px;">
                        <div style="font-weight: bold; color: {priority_color};">{alert['title']}</div>
                        <div style="font-size: 0.7rem; color: #666;">{alert['timestamp']} | {alert['type'].replace('_', ' ').title()}</div>
